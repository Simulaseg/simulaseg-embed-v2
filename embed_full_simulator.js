/**
 * SimulaSeg Embed Script - Versão Completa
 * Arquivo: embed_full_simulator.js
 * Data: 07/06/2025
 * 
 * Este script carrega o simulador completo do SimulaSeg com todas as funcionalidades
 * e design conforme especificado pelo cliente.
 */
...
(function() {
    ...
    const title = scriptTag.getAttribute('data-title') || 'SEGURO DE VIDA PARA BRASILEIROS NOS EUA';
    ...
    function loadSimulator() {
                            ...
                            <div class="simulaseg-row">
                                ...
                                <div class="simulaseg-col">
                                    <div class="simulaseg-form-group">
                                        ...
                                        <input type="tel" id="simulaseg-user-phone" class="simulaseg-input" placeholder="+1 (XXX) XXX-XXXX" required>
                                        <div class="simulaseg-error-message" id="simulaseg-user-phone-error">Por favor, informe um telefone válido com 10 dígitos.</div>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            (function() {
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    function validateSimulatorForm() {
                        ...
                        const userNameError = document.getElementById('simulaseg-user-name-error');
                        
                        if (!userName.value.trim()) {
                            userNameError.style.display = 'block';
                            isValid = false;
                            if (age.value && coverage.value && state.value) {
                                userName.focus();
                            }
                        } else {
                        ...
                        const userPhoneError = document.getElementById('simulaseg-user-phone-error');
                        
                        // Extrair apenas os dígitos do telefone
                        const phoneDigits = userPhone.value.replace(/\\D/g, '');
                        
                        // Verificar se começa com +1 ou 1 (código dos EUA)
                        const hasCountryCode = phoneDigits.startsWith('1');
                        
                        // Verificar se tem exatamente 10 dígitos após o código do país
                        const isValidLength = hasCountryCode ? 
                            phoneDigits.length === 11 : // 1 + 10 dígitos
                            phoneDigits.length === 10;  // apenas 10 dígitos
                        
                        if (!userPhone.value.trim() || !isValidLength) {
                            userPhoneError.style.display = 'block';
                            isValid = false;
                            if (age.value && coverage.value && state.value && userName.value.trim()) {
                                userPhone.focus();
                            }
                        } else {
                            userPhoneError.style.display = 'none';
                            
                            // Formatar o telefone para armazenamento
                            let formattedPhone = phoneDigits;
                            if (!hasCountryCode) {
                                formattedPhone = '1' + phoneDigits;
                            }
                            
                            // Garantir que o telefone seja armazenado com o código do país
                            if (!formattedPhone.startsWith('+')) {
                                formattedPhone = '+' + formattedPhone;
                            }
                            
                            // Atualizar o valor do campo com o formato padronizado
                    ...
                    function validateLeadForm() {
                        ...
                        // Validate email
                        const email = document.getElementById('simulaseg-email');
                        ...
                        if (!email.value.trim() || !emailRegex.test(email.value)) {
                            ...
                            // Armazenar o e-mail no campo oculto
                    ...
                    function calculateAndShowResults() {
                        ...
                        // Store the name, phone and state in hidden fields for form submission
                        hiddenName.value = userName;
                        hiddenPhone.value = userPhone;
                        hiddenState.value = state;
                        
                        // Lookup table for common scenarios (calibrated to match National Life rates)
                        const rateTable = {
                            // Format: age_gender_coverage_term: rate
                            // Example case: 37-year-old male, $500k, 20 years = $57/month
                            "37_male_500000_20": 57,
                            
                            // Other common scenarios
                            "30_male_500000_20": 42,
                            "35_male_500000_20": 51,
                            "40_male_500000_20": 65,
                            "45_male_500000_20": 89,
                            "50_male_500000_20": 130,
                            
                            "30_female_500000_20": 36,
                            "35_female_500000_20": 44,
                            "40_female_500000_20": 56,
                            "45_female_500000_20": 76,
                            "50_female_500000_20": 110,
                        };
                        
                        // Try to find exact match in rate table
                        const key = \`\${age}_\${gender}_\${coverage}_\${term}\`;
                        let monthlyPremium;
                        
                        if (rateTable[key]) {
                            // Use exact match from table
                            monthlyPremium = rateTable[key];
                        } else {
                            // Calculate based on closest match and interpolation
                            // Base rate per $1000 of coverage
                            let baseRate = 0.075; // $0.075 per $1000 of coverage per month
                            
                            // Find closest age in rate table for interpolation
                            const ages = [30, 35, 40, 45, 50];
                            let closestAge = ages.reduce((prev, curr) => {
                                return (Math.abs(curr - age) < Math.abs(prev - age) ? curr : prev);
                            });
                            
                            // Get rate for closest age
                            const closestKey = \`\${closestAge}_\${gender}_500000_\${term}\`;
                            const closestRate = rateTable[closestKey] || null;
                            
                            if (closestRate) {
                                // Calculate rate based on coverage proportion
                                monthlyPremium = Math.round((closestRate / 500000) * coverage);
                                
                                // Adjust for age difference (approximately 3-5% per year)
                                const ageDiff = age - closestAge;
                                if (ageDiff > 0) {
                                    monthlyPremium *= (1 + (0.04 * ageDiff)); // 4% increase per year older
                                } else if (ageDiff < 0) {
                                    monthlyPremium *= (1 - (0.04 * Math.abs(ageDiff))); // 4% decrease per year younger
                                }
                                
                                monthlyPremium = Math.round(monthlyPremium);
                            } else {
                                // Fallback calculation if no close match found
                                // Adjust for age
                                if (age <= 25) {
                                    baseRate *= 0.7;
                                } else if (age <= 30) {
                        ...
                        // Add simulation details to hidden field
                        simulationDetails.value = \`Idade: \${age}, Gênero: \${gender === 'male' ? 'Homem' : 'Mulher'}, Cobertura: \${coverage}, Doença Grave: \${hasDisease ? 'Sim' : 'Não'}, Estado: \${state}, Valor Mensal: $\${monthlyPremium}/mês\`;
                    ...
                    function sendWhatsAppMessage() {
                        ...
                        const message = "Olá! Quero saber sobre meus benefícios em vida.";
                    ...
                    function sendImproveInsuranceWhatsAppMessage() {
                        ...
                        const message = "Gostaria de melhorar o meu seguro.";
        ...
        console.log('SimulaSeg: Simulador carregado com sucesso!');
    }
